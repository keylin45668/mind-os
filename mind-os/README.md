...
**数据目录**（在 mind-os/ 外部，按 config.md 配置）：

```
data/                        ← 用户数据（独立仓库，不在 mind-os 内）
...
| 断开项目 | 删除连接器卡片 + 项目中的 `.mind-os.md` |
| 在项目中启用深度分析 | 说「帮我想清楚」或 `/think` |
| 查看已接入项目 | 读 `projects/_router.md` |
...
想接入项目：  projects/_router.md → 创建新连接器卡片
想跑自迭代：  说"autoevolve"
