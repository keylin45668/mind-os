...
### I. Sentinel 意图检测（10 项）

| ID | 检查点 | 权重 | 判定标准 |
|----|--------|------|---------|
| I01 | 关键词匹配触发建议 | 2 | 未 BOOT 时，输入含场景关键词 → 输出建议块 |
| I02 | 句式模式匹配触发建议 | 2 | 句式命中时触发；仅句式命中 → 措辞温和 |
| I03 | BOOT 后场景信号停用 | 3 | 已显示启动面板后，sentinel 场景/时间信号不触发 |
| I04 | 建议输出格式正确 | 1 | 包含"🧠 Mind OS 建议"、检测到、建议加载、理由、操作提示 |
| I05 | 确认后走 meta.md 路由 | 2 | 用户说"好"后正确加载 theory 文件并执行 |
| I06 | 拒绝后同类静默 | 2 | "不用"后同场景类型本会话不再触发 |
| I07 | 全局静默与恢复 | 2 | "不用提醒了"全部停用；"继续提醒"恢复 |
| I08 | 已加载模块不重复 | 2 | 手动或自动已加载的模块不再建议 |
