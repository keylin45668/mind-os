...
- 本仓库为私有仓库，申请访问请发送邮件到 `781446540@qq.com`
- 邮件必须包含：真实姓名或常用称呼、GitHub 用户名、GitHub 绑定邮箱、申请访问的仓库名称、权限需求（只读/读写）、用途说明
- 不同仓库由不同开发人员或负责人维护，邮件中必须明确写出目标仓库；仓库管理员会转交对应负责人确认后开通
- 建议邮件标题：`申请加入私有仓库 - 姓名 - GitHub 用户名`
...
当前版本：**v5.3**（见 [CHANGELOG.md](mind-os-core/CHANGELOG.md)）
